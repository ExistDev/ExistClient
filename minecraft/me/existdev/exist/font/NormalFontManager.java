...
public class NormalFontManager extends FontRenderer {

	public final Random fontRandom = new Random();
	private final Color[] customColorCodes = new Color[256];
	private final int[] colorCode = new int[32];
	private NormalFont font, boldFont, italicFont, boldItalicFont;
	private String colorcodeIdentifiers = "0123456789abcdefklmnor";
	private boolean bidi;

	public NormalFontManager(final Font font, final boolean antiAlias, final int charOffset) {
	...
	public int drawString(final String s, final double x, final double y, final int color) {
		return drawString(s, x, y, color, false);
	}
	...
	public int drawString(final String text, double x, double y, int color, final boolean shadow) {
		int result = 0;
	...
	private int drawLine(final String text, double x, double y, int color, boolean shadow) {
		if (text == null)
			return 0;
		glPushMatrix();
		glTranslated(x - 1.5, y + 0.5, 0);
	 ...
	 * Make dis
	 */
	private String toRandom(final NormalFont font, final String text) {
		String newText = "";
	...
	/**
	 * Instantiates the FancyFont objects that will be used to render the shit.
	 */
	public void setFont(final Font font, final boolean antiAlias, final int charOffset) {
		synchronized (this) {
			this.font = new NormalFont(font, antiAlias, charOffset);
			boldFont = new NormalFont(font.deriveFont(Font.BOLD), antiAlias, charOffset);
			italicFont = new NormalFont(font.deriveFont(Font.ITALIC), antiAlias, charOffset);
			boldItalicFont = new NormalFont(font.deriveFont(Font.BOLD | Font.ITALIC), antiAlias, charOffset);
			FONT_HEIGHT = getHeight();
		}
	}

	/**
	 * @return FancyFont instance.
	 */
	public NormalFont getFont() {
		return font;
	}
